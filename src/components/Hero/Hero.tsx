...
type LegendColors = 'green-400' | 'blue-400' | 'purple-400' | 'orange-400';
...
const Hero = () => {
  ...
  const lengendMap: ILegendItem[] = [
    { title: 'TypeScript Ready', color: 'green-400' },
    { title: 'Tailwind CSS', color: 'blue-400' },
    { title: 'Multiple Frameworks', color: 'purple-400' },
    { title: 'Production Ready', color: 'orange-400' },
  ];
          ...
          {lengendMap.map((el, idx) => {
            return (
              <div key={idx} className='flex items-center gap-2'>
                <div
                  className={`bg-${el.color} rounded-full h-1.5 w-1.5`}
                ></div>
